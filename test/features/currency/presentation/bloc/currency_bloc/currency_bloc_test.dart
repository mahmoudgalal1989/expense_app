...
class MockGetAllCurrencies extends Mock
    implements Command<List<Currency>, NoParams> {}

class MockSetSelectedCurrency extends Mock
    implements Command<void, SetSelectedCurrencyParams> {}

class MockSearchCurrencies extends Mock
    implements Command<List<Currency>, search_usecase.SearchCurrenciesParams> {}

// Fake classes for fallback values
class FakeSetSelectedCurrencyParams extends Fake
    implements SetSelectedCurrencyParams {}

class FakeSearchCurrenciesParams extends Fake
    implements search_usecase.SearchCurrenciesParams {}
...
void main() {
  ...
  group('LoadCurrencies', () {
    test(
      ...
      () async {
        ...
        expect(loadedState.mostUsedCurrencies,
    ...
    test(
      ...
      () async {
        ...
        when(() => mockGetAllCurrencies(any())).thenAnswer((_) async =>
            const Left<Failure, List<Currency>>(
                CacheFailure('Failed to load')));
  ...
  group('SearchCurrencies', () {
    ...
    test('should call the search use case with the query and update state',
        () async {
      // arrange
      when(() => mockSearchCurrencies(any()))
    ...
    test('should update state with error when search fails', () async {
      ...
      when(() => mockSearchCurrencies(any())).thenAnswer((_) async =>
          const Left<Failure, List<Currency>>(ServerFailure(tErrorMessage)));
  ...
  group('SelectCurrency', () {
    ...
    test('should update selected currency when a valid currency is selected',
        () async {
      ...
      final state = await testBloc.stream.firstWhere(
        (state) =>
            ...
            state.selectedCurrency.code == newCurrency.code,
        orElse: () => testBloc.state,
      );
